...
ALRS uses two point types—**QRP** and **MRP**—plus a reward-unit called an **ARC** (AsBeez Reward Cell). Together, they create a system that is transparent to members (they can see their points and progress) while keeping sensitive margin data internal.
...
**ARC** is the renamed concept formerly called “ARC.” An ARC is a **loyalty reward unit** unlocked through shopping activity. Customers can own **unlimited ARCs**.

ARCs are placed into a structured network called the **ARC Grid** (a forced 3-wide matrix). An ARC may receive MRP rewards if it is considered **eligible (active)**.
...
- Used to trigger ARC creation: **every 100 QRP → 1 ARC minted**.
- QRP is **not** earned from being an upline ARC.
...
- **Does not** unlock ARCs and **never** contributes to QRP thresholds.

> Think of MRP like a “reward wallet balance.”
...
ALRS computes QRP from a **margin snapshot** stored internally per order item:
...
Customers only see:
- QRP awarded per receipt/order
- QRP lifetime totals and progress
- MRP wallet and store credit results

### 3.2 QRP calculation rule (internal)
...
- QRP awarded is typically an integer (rounded down) to avoid over-crediting.
...
- **Every 100 QRP earned by a customer unlocks 1 ARC**.
- Customers can mint **unlimited ARCs** over time as they continue shopping.

### 4.2 QRP lifetime and “unlocked” tracking
To ensure ARCs are minted correctly, the system tracks:
- **QRP lifetime**: total QRP earned from purchases
- **QRP unlocked**: total QRP already used to mint ARCs in 100-point increments

Example:
- QRP lifetime = 260
- QRP unlocked = 200
- Newly mintable = 60 (not enough)
- If QRP lifetime becomes 320 → eligible = 300; newly mintable = 100 → mint 1 ARC

### 4.3 No “ARC bonus points”
Creating an ARC does **not** automatically award the owner extra points.
- No “new ARC reward” to the owner
- Rewards are handled via MRP distribution rules (next section)

---

## 5. MRP allocation when an ARC is minted (60/40 rule)

Each time **1 ARC is minted**, the system creates **100 MRP** and allocates it:

- **60 MRP**: intended for upline rewards (up to 12 eligible uplines × 5 MRP each)
- **40 MRP**: goes directly to **Portal Reserve** (operations budget)

This is the baseline ALRS economic split used to fund:
- community rewards (upline MRP)
- platform salaries, perks, maintenance, and operations (portal reserve)

---

## 6. Upline MRP rewards on placement (eligibility + skipping)
...
- Each eligible upline receives: **5 MRP**
- Maximum upline distribution: **60 MRP**
...
- **Unused MRP from the 60 MRP upline budget transfers immediately to Portal Reserve.**
...
Result:
- Unused portion of the 60 MRP upline budget is **redirected to Portal Reserve immediately**.

### 7.2 Liquidity shortage (pool temporarily lacks balance)
Cause: eligible uplines exist, but the **Global Reward Pool** doesn’t have enough MRP at that moment.
Result:
...
### 8.1 Store credit is funded by MRP only
- **MRP** can be converted to **AsBeez store credit**.
- **QRP cannot be converted**; it only unlocks ARCs.
...
- MRP may start as **pending** and become **available** after a holding period (e.g., 15–30 days).
...
Reinforced rule:
- If a purchase uses store credit, it earns **no QRP** and **no MRP**.
...
- QRP earned per receipt/order
- QRP lifetime and progress to next ARC
- Total ARCs owned
- MRP pending/available balances
...
- **Customer:** QR, QRP/MRP balances, receipts, claims, store credit
...
3) Purchases award **QRP** (progress points).
4) Every **100 QRP** unlocks a new **ARC**.
5) Each ARC minted adds **MRP** to the system:
   - Up to 60 MRP intended for community rewards
   - 40 MRP to portal reserve
6) When ARCs are placed, eligible active uplines can receive **MRP**.
7) MRP can be converted into **AsBeez store credit** (after rules/approval).
...
- Use a customer-safe “receipt points summary” record so customers can see their earned QRP per order without exposing internal margin.
